...
import CameraSvgIcon from "@/assets/svg/CameraSvgIcon.vue";
...
<template>
  ...
  <div
    class="overflow-hidden bg-white border rounded-sm border-stroke shadow-default dark:border-strokedark dark:bg-boxdark"
  >
    <div class="relative z-20 h-35 md:h-65">
      ...
      />
      <div class="absolute z-10 bottom-1 right-1 xsm:bottom-4 xsm:right-4">
        <Link
          :href="route('profile.settings')"
          class="flex items-center justify-center gap-2 px-2 py-1 text-sm font-medium text-white rounded cursor-pointer bg-primary hover:bg-opacity-80 xsm:px-4"
        >
          <span>
            <!-- <CameraSvgIcon /> -->
    ...
    <div class="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
      <div
        class="relative z-30 w-full p-1 mx-auto rounded-full -mt-22 h-30 max-w-30 bg-white/20 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3"
      >
        <div class="relative drop-shadow-2" v-show="!photoPreview">
          <img :src="user.profile_photo_url" alt="profile" class="w-full h-full rounded-full" />
          <!-- <label
            for="profile"
        ...
        <div class="relative drop-shadow-2" v-show="photoPreview">
          <div class="">
            <span
              class="block sm:w-[9.4rem] sm:h-[9.4rem] w-[7rem] h-[7rem] bg-center bg-no-repeat bg-cover rounded-full"
      ...
      <div class="mt-4">
        ...
        >
          <div
            class="px-4 border-stroke dark:border-strokedark"
          >
