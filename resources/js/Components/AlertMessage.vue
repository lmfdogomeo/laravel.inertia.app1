<script setup>
import { nextTick, onMounted, ref } from "vue";

const alertOpen = ref(false);
const props = defineProps({
  type: String,
  message: String,
  title: String,
  duration: Number,
});
const type = ref(null);
const message = ref(null);
const title = ref(null);
const duration = ref(2000);
const timeout = ref(null);

const closeAlert = () => {
  alertOpen.value = false;
};

const show = (params = {}) => {
  alertOpen.value = true;
  type.value = params.type || type.value;
  message.value = params.message || message.value;
  title.value = params.title || title.value;
  duration.value = params.duration || duration.value;

  clearTimeout(timeout.value);

  nextTick(() => {
    timeout.value = setTimeout(() => {
      alertOpen.value = false;
    }, duration.value);
  });
};

defineExpose({
  show,
});

onMounted(() => {
  type.value = props.type || type.value;
  message.value = props.message || message.value;
  title.value = props.title || title.value;
  duration.value = props.duration || duration.value;
});
</script>
<template>
  <transition
    enter-from-class="translate-x-[100%] opacity-0"
    leave-to-class="translate-x-[100%] opacity-0"
    enter-active-class="transition duration-300"
    leave-active-class="transition duration-300"
  >
    <div
      v-show="alertOpen"
      class="bg-white shadow-[rgba(13,_38,_76,_0.19)_0px_9px_20px] fixed top-5 right-5 z-[9999]"
    >
      <div
        class="flex w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9"
      >
        <div
          class="mr-5 flex h-9 w-full max-w-[36px] items-center justify-center rounded-lg bg-[#34D399]"
        >
          <svg
            width="16"
            height="12"
            viewBox="0 0 16 12"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M15.2984 0.826822L15.2868 0.811827L15.2741 0.797751C14.9173 0.401867 14.3238 0.400754 13.9657 0.794406L5.91888 9.45376L2.05667 5.2868C1.69856 4.89287 1.10487 4.89389 0.747996 5.28987C0.417335 5.65675 0.417335 6.22337 0.747996 6.59026L0.747959 6.59029L0.752701 6.59541L4.86742 11.0348C5.14445 11.3405 5.52858 11.5 5.89581 11.5C6.29242 11.5 6.65178 11.3355 6.92401 11.035L15.2162 2.11161C15.5833 1.74452 15.576 1.18615 15.2984 0.826822Z"
              fill="white"
              stroke="white"
            ></path>
          </svg>
        </div>
        <div class="w-full">
          <h5 class="mb-3 text-lg font-bold text-black dark:text-[#34D399]">
            Message Sent Successfully
          </h5>
          <p class="text-base leading-relaxed text-body">
            Lorem Ipsum is simply dummy text of the printing and typesetting
            industry.
          </p>
        </div>
      </div>
    </div>
  </transition>
</template>
